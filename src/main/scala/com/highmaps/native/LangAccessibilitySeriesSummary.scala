/**
  * Automatically generated file. Please do not edit.
  * @author Highcharts Config Generator by Karasiq
  * @see [[http://api.highcharts.com/highmaps]]
  */
package com.highmaps.native

import scalajs.js, js.`|`
import org.scalajs.dom

import com.highcharts.CleanJsObject
import com.highcharts.HighchartsUtils._

/**
  * @note JavaScript name: <code>lang-accessibility-series-summary</code>
  */
class LangAccessibilitySeriesSummary extends com.highcharts.HighchartsGenericObject {

  var bar: js.UndefOr[String] = js.undefined

  var barCombination: js.UndefOr[String] = js.undefined

  var boxplot: js.UndefOr[String] = js.undefined

  var boxplotCombination: js.UndefOr[String] = js.undefined

  var bubble: js.UndefOr[String] = js.undefined

  var bubbleCombination: js.UndefOr[String] = js.undefined

  var column: js.UndefOr[String] = js.undefined

  var columnCombination: js.UndefOr[String] = js.undefined

  var default: js.UndefOr[String] = js.undefined

  var defaultCombination: js.UndefOr[String] = js.undefined

  var line: js.UndefOr[String] = js.undefined

  var lineCombination: js.UndefOr[String] = js.undefined

  var map: js.UndefOr[String] = js.undefined

  var mapCombination: js.UndefOr[String] = js.undefined

  var mapbubble: js.UndefOr[String] = js.undefined

  var mapbubbleCombination: js.UndefOr[String] = js.undefined

  var mapline: js.UndefOr[String] = js.undefined

  var maplineCombination: js.UndefOr[String] = js.undefined

  var pie: js.UndefOr[String] = js.undefined

  var pieCombination: js.UndefOr[String] = js.undefined

  var scatter: js.UndefOr[String] = js.undefined

  var scatterCombination: js.UndefOr[String] = js.undefined

  var spline: js.UndefOr[String] = js.undefined

  var splineCombination: js.UndefOr[String] = js.undefined
}

object LangAccessibilitySeriesSummary {
  /**
    */
  def apply(bar: js.UndefOr[String] = js.undefined, barCombination: js.UndefOr[String] = js.undefined, boxplot: js.UndefOr[String] = js.undefined, boxplotCombination: js.UndefOr[String] = js.undefined, bubble: js.UndefOr[String] = js.undefined, bubbleCombination: js.UndefOr[String] = js.undefined, column: js.UndefOr[String] = js.undefined, columnCombination: js.UndefOr[String] = js.undefined, default: js.UndefOr[String] = js.undefined, defaultCombination: js.UndefOr[String] = js.undefined, line: js.UndefOr[String] = js.undefined, lineCombination: js.UndefOr[String] = js.undefined, map: js.UndefOr[String] = js.undefined, mapCombination: js.UndefOr[String] = js.undefined, mapbubble: js.UndefOr[String] = js.undefined, mapbubbleCombination: js.UndefOr[String] = js.undefined, mapline: js.UndefOr[String] = js.undefined, maplineCombination: js.UndefOr[String] = js.undefined, pie: js.UndefOr[String] = js.undefined, pieCombination: js.UndefOr[String] = js.undefined, scatter: js.UndefOr[String] = js.undefined, scatterCombination: js.UndefOr[String] = js.undefined, spline: js.UndefOr[String] = js.undefined, splineCombination: js.UndefOr[String] = js.undefined): LangAccessibilitySeriesSummary = {
    com.highcharts.HighchartsGenericObject.toCleanObject(new LangAccessibilitySeriesSummary {
      this.bar = bar
      this.barCombination = barCombination
      this.boxplot = boxplot
      this.boxplotCombination = boxplotCombination
      this.bubble = bubble
      this.bubbleCombination = bubbleCombination
      this.column = column
      this.columnCombination = columnCombination
      this.default = default
      this.defaultCombination = defaultCombination
      this.line = line
      this.lineCombination = lineCombination
      this.map = map
      this.mapCombination = mapCombination
      this.mapbubble = mapbubble
      this.mapbubbleCombination = mapbubbleCombination
      this.mapline = mapline
      this.maplineCombination = maplineCombination
      this.pie = pie
      this.pieCombination = pieCombination
      this.scatter = scatter
      this.scatterCombination = scatterCombination
      this.spline = spline
      this.splineCombination = splineCombination
    })
  }
}
