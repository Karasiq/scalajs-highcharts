...
object Chart {
  /**
    ...
    * @param renderTo <p>The HTML element where the chart will be rendered. If it is a string, the element by that id is used. The HTML element can also be passed by direct reference.</p>. . <p>When using the <a href="#Highcharts.chart">Highcharts.chart</a> constructor, the <code>renderTo</code> is given as a parameter, and can be skipped in the options set.</p>
