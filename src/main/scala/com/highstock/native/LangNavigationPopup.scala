/**
  * Automatically generated file. Please do not edit.
  * @author Highcharts Config Generator by Karasiq
  * @see [[http://api.highcharts.com/highstock]]
  */
package com.highstock.native

import scalajs.js, js.`|`
import org.scalajs.dom

import com.highcharts.CleanJsObject
import com.highcharts.HighchartsUtils._

/**
  * @note JavaScript name: <code>lang-navigation-popup</code>
  */
class LangNavigationPopup extends com.highcharts.HighchartsGenericObject {

  var addButton: js.UndefOr[String] = js.undefined

  var algorithm: js.UndefOr[String] = js.undefined

  var arrowInfinityLine: js.UndefOr[String] = js.undefined

  var arrowRay: js.UndefOr[String] = js.undefined

  var arrowSegment: js.UndefOr[String] = js.undefined

  var average: js.UndefOr[String] = js.undefined

  var background: js.UndefOr[String] = js.undefined

  var backgroundColor: js.UndefOr[String] = js.undefined

  var backgroundColors: js.UndefOr[String] = js.undefined

  var borderColor: js.UndefOr[String] = js.undefined

  var borderRadius: js.UndefOr[String] = js.undefined

  var borderWidth: js.UndefOr[String] = js.undefined

  var bottomBand: js.UndefOr[String] = js.undefined

  var circle: js.UndefOr[String] = js.undefined

  var clearFilter: js.UndefOr[String] = js.undefined

  var color: js.UndefOr[String] = js.undefined

  var connector: js.UndefOr[String] = js.undefined

  var crooked3: js.UndefOr[String] = js.undefined

  var crooked5: js.UndefOr[String] = js.undefined

  var crosshairX: js.UndefOr[String] = js.undefined

  var crosshairY: js.UndefOr[String] = js.undefined

  var decimals: js.UndefOr[String] = js.undefined

  var deviation: js.UndefOr[String] = js.undefined

  var editButton: js.UndefOr[String] = js.undefined

  var elliott3: js.UndefOr[String] = js.undefined

  var elliott5: js.UndefOr[String] = js.undefined

  var ellipse: js.UndefOr[String] = js.undefined

  var factor: js.UndefOr[String] = js.undefined

  var fastAvgPeriod: js.UndefOr[String] = js.undefined

  var fibonacci: js.UndefOr[String] = js.undefined

  var fibonacciTimeZones: js.UndefOr[String] = js.undefined

  var fill: js.UndefOr[String] = js.undefined

  var flags: js.UndefOr[String] = js.undefined

  var fontSize: js.UndefOr[String] = js.undefined

  var format: js.UndefOr[String] = js.undefined

  var height: js.UndefOr[String] = js.undefined

  var highIndex: js.UndefOr[String] = js.undefined

  var horizontalLine: js.UndefOr[String] = js.undefined

  var increment: js.UndefOr[String] = js.undefined

  var index: js.UndefOr[String] = js.undefined

  /**
    * <p>Configure the aliases for indicator names.</p>
    * @since 9.3.0
    */
  var indicatorAliases: js.Any = js.undefined

  var infinityLine: js.UndefOr[String] = js.undefined

  var initialAccelerationFactor: js.UndefOr[String] = js.undefined

  var innerBackground: js.UndefOr[String] = js.undefined

  var label: js.UndefOr[String] = js.undefined

  var labelOptions: js.UndefOr[String] = js.undefined

  var labels: js.UndefOr[String] = js.undefined

  var line: js.UndefOr[String] = js.undefined

  var lines: js.UndefOr[String] = js.undefined

  var longPeriod: js.UndefOr[String] = js.undefined

  var lowIndex: js.UndefOr[String] = js.undefined

  var maxAccelerationFactor: js.UndefOr[String] = js.undefined

  var measure: js.UndefOr[String] = js.undefined

  var measureX: js.UndefOr[String] = js.undefined

  var measureXY: js.UndefOr[String] = js.undefined

  var measureY: js.UndefOr[String] = js.undefined

  var multiplier: js.UndefOr[String] = js.undefined

  var multiplierATR: js.UndefOr[String] = js.undefined

  var name: js.UndefOr[String] = js.undefined

  var noFilterMatch: js.UndefOr[String] = js.undefined

  var outerBackground: js.UndefOr[String] = js.undefined

  var padding: js.UndefOr[String] = js.undefined

  var parallelChannel: js.UndefOr[String] = js.undefined

  var period: js.UndefOr[String] = js.undefined

  var periodATR: js.UndefOr[String] = js.undefined

  var periodSenkouSpanB: js.UndefOr[String] = js.undefined

  var periodTenkan: js.UndefOr[String] = js.undefined

  var periods: js.UndefOr[String] = js.undefined

  var pitchfork: js.UndefOr[String] = js.undefined

  var ranges: js.UndefOr[String] = js.undefined

  var ray: js.UndefOr[String] = js.undefined

  var rectangle: js.UndefOr[String] = js.undefined

  var removeButton: js.UndefOr[String] = js.undefined

  var saveButton: js.UndefOr[String] = js.undefined

  var searchIndicators: js.UndefOr[String] = js.undefined

  var segment: js.UndefOr[String] = js.undefined

  var series: js.UndefOr[String] = js.undefined

  var shapeOptions: js.UndefOr[String] = js.undefined

  var shapes: js.UndefOr[String] = js.undefined

  var shortPeriod: js.UndefOr[String] = js.undefined

  var signalPeriod: js.UndefOr[String] = js.undefined

  var simpleShapes: js.UndefOr[String] = js.undefined

  var slowAvgPeriod: js.UndefOr[String] = js.undefined

  var standardDeviation: js.UndefOr[String] = js.undefined

  var stroke: js.UndefOr[String] = js.undefined

  var strokeWidth: js.UndefOr[String] = js.undefined

  var style: js.UndefOr[String] = js.undefined

  var timeCycles: js.UndefOr[String] = js.undefined

  var title: js.UndefOr[String] = js.undefined

  var topBand: js.UndefOr[String] = js.undefined

  var tunnel: js.UndefOr[String] = js.undefined

  var typeOptions: js.UndefOr[String] = js.undefined

  var verticalArrow: js.UndefOr[String] = js.undefined

  var verticalCounter: js.UndefOr[String] = js.undefined

  var verticalLabel: js.UndefOr[String] = js.undefined

  var verticalLine: js.UndefOr[String] = js.undefined

  var volume: js.UndefOr[String] = js.undefined

  var xAxisUnit: js.UndefOr[String] = js.undefined
}

object LangNavigationPopup {
  /**
    * @param indicatorAliases <p>Configure the aliases for indicator names.</p>
    */
  def apply(addButton: js.UndefOr[String] = js.undefined, algorithm: js.UndefOr[String] = js.undefined, arrowInfinityLine: js.UndefOr[String] = js.undefined, arrowRay: js.UndefOr[String] = js.undefined, arrowSegment: js.UndefOr[String] = js.undefined, average: js.UndefOr[String] = js.undefined, background: js.UndefOr[String] = js.undefined, backgroundColor: js.UndefOr[String] = js.undefined, backgroundColors: js.UndefOr[String] = js.undefined, borderColor: js.UndefOr[String] = js.undefined, borderRadius: js.UndefOr[String] = js.undefined, borderWidth: js.UndefOr[String] = js.undefined, bottomBand: js.UndefOr[String] = js.undefined, circle: js.UndefOr[String] = js.undefined, clearFilter: js.UndefOr[String] = js.undefined, color: js.UndefOr[String] = js.undefined, connector: js.UndefOr[String] = js.undefined, crooked3: js.UndefOr[String] = js.undefined, crooked5: js.UndefOr[String] = js.undefined, crosshairX: js.UndefOr[String] = js.undefined, crosshairY: js.UndefOr[String] = js.undefined, decimals: js.UndefOr[String] = js.undefined, deviation: js.UndefOr[String] = js.undefined, editButton: js.UndefOr[String] = js.undefined, elliott3: js.UndefOr[String] = js.undefined, elliott5: js.UndefOr[String] = js.undefined, ellipse: js.UndefOr[String] = js.undefined, factor: js.UndefOr[String] = js.undefined, fastAvgPeriod: js.UndefOr[String] = js.undefined, fibonacci: js.UndefOr[String] = js.undefined, fibonacciTimeZones: js.UndefOr[String] = js.undefined, fill: js.UndefOr[String] = js.undefined, flags: js.UndefOr[String] = js.undefined, fontSize: js.UndefOr[String] = js.undefined, format: js.UndefOr[String] = js.undefined, height: js.UndefOr[String] = js.undefined, highIndex: js.UndefOr[String] = js.undefined, horizontalLine: js.UndefOr[String] = js.undefined, increment: js.UndefOr[String] = js.undefined, index: js.UndefOr[String] = js.undefined, indicatorAliases: js.UndefOr[js.Any] = js.undefined, infinityLine: js.UndefOr[String] = js.undefined, initialAccelerationFactor: js.UndefOr[String] = js.undefined, innerBackground: js.UndefOr[String] = js.undefined, label: js.UndefOr[String] = js.undefined, labelOptions: js.UndefOr[String] = js.undefined, labels: js.UndefOr[String] = js.undefined, line: js.UndefOr[String] = js.undefined, lines: js.UndefOr[String] = js.undefined, longPeriod: js.UndefOr[String] = js.undefined, lowIndex: js.UndefOr[String] = js.undefined, maxAccelerationFactor: js.UndefOr[String] = js.undefined, measure: js.UndefOr[String] = js.undefined, measureX: js.UndefOr[String] = js.undefined, measureXY: js.UndefOr[String] = js.undefined, measureY: js.UndefOr[String] = js.undefined, multiplier: js.UndefOr[String] = js.undefined, multiplierATR: js.UndefOr[String] = js.undefined, name: js.UndefOr[String] = js.undefined, noFilterMatch: js.UndefOr[String] = js.undefined, outerBackground: js.UndefOr[String] = js.undefined, padding: js.UndefOr[String] = js.undefined, parallelChannel: js.UndefOr[String] = js.undefined, period: js.UndefOr[String] = js.undefined, periodATR: js.UndefOr[String] = js.undefined, periodSenkouSpanB: js.UndefOr[String] = js.undefined, periodTenkan: js.UndefOr[String] = js.undefined, periods: js.UndefOr[String] = js.undefined, pitchfork: js.UndefOr[String] = js.undefined, ranges: js.UndefOr[String] = js.undefined, ray: js.UndefOr[String] = js.undefined, rectangle: js.UndefOr[String] = js.undefined, removeButton: js.UndefOr[String] = js.undefined, saveButton: js.UndefOr[String] = js.undefined, searchIndicators: js.UndefOr[String] = js.undefined, segment: js.UndefOr[String] = js.undefined, series: js.UndefOr[String] = js.undefined, shapeOptions: js.UndefOr[String] = js.undefined, shapes: js.UndefOr[String] = js.undefined, shortPeriod: js.UndefOr[String] = js.undefined, signalPeriod: js.UndefOr[String] = js.undefined, simpleShapes: js.UndefOr[String] = js.undefined, slowAvgPeriod: js.UndefOr[String] = js.undefined, standardDeviation: js.UndefOr[String] = js.undefined, stroke: js.UndefOr[String] = js.undefined, strokeWidth: js.UndefOr[String] = js.undefined, style: js.UndefOr[String] = js.undefined, timeCycles: js.UndefOr[String] = js.undefined, title: js.UndefOr[String] = js.undefined, topBand: js.UndefOr[String] = js.undefined, tunnel: js.UndefOr[String] = js.undefined, typeOptions: js.UndefOr[String] = js.undefined, verticalArrow: js.UndefOr[String] = js.undefined, verticalCounter: js.UndefOr[String] = js.undefined, verticalLabel: js.UndefOr[String] = js.undefined, verticalLine: js.UndefOr[String] = js.undefined, volume: js.UndefOr[String] = js.undefined, xAxisUnit: js.UndefOr[String] = js.undefined): LangNavigationPopup = {
    com.highcharts.HighchartsGenericObject.toCleanObject(new LangNavigationPopup {
      this.addButton = addButton
      this.algorithm = algorithm
      this.arrowInfinityLine = arrowInfinityLine
      this.arrowRay = arrowRay
      this.arrowSegment = arrowSegment
      this.average = average
      this.background = background
      this.backgroundColor = backgroundColor
      this.backgroundColors = backgroundColors
      this.borderColor = borderColor
      this.borderRadius = borderRadius
      this.borderWidth = borderWidth
      this.bottomBand = bottomBand
      this.circle = circle
      this.clearFilter = clearFilter
      this.color = color
      this.connector = connector
      this.crooked3 = crooked3
      this.crooked5 = crooked5
      this.crosshairX = crosshairX
      this.crosshairY = crosshairY
      this.decimals = decimals
      this.deviation = deviation
      this.editButton = editButton
      this.elliott3 = elliott3
      this.elliott5 = elliott5
      this.ellipse = ellipse
      this.factor = factor
      this.fastAvgPeriod = fastAvgPeriod
      this.fibonacci = fibonacci
      this.fibonacciTimeZones = fibonacciTimeZones
      this.fill = fill
      this.flags = flags
      this.fontSize = fontSize
      this.format = format
      this.height = height
      this.highIndex = highIndex
      this.horizontalLine = horizontalLine
      this.increment = increment
      this.index = index
      this.indicatorAliases = indicatorAliases
      this.infinityLine = infinityLine
      this.initialAccelerationFactor = initialAccelerationFactor
      this.innerBackground = innerBackground
      this.label = label
      this.labelOptions = labelOptions
      this.labels = labels
      this.line = line
      this.lines = lines
      this.longPeriod = longPeriod
      this.lowIndex = lowIndex
      this.maxAccelerationFactor = maxAccelerationFactor
      this.measure = measure
      this.measureX = measureX
      this.measureXY = measureXY
      this.measureY = measureY
      this.multiplier = multiplier
      this.multiplierATR = multiplierATR
      this.name = name
      this.noFilterMatch = noFilterMatch
      this.outerBackground = outerBackground
      this.padding = padding
      this.parallelChannel = parallelChannel
      this.period = period
      this.periodATR = periodATR
      this.periodSenkouSpanB = periodSenkouSpanB
      this.periodTenkan = periodTenkan
      this.periods = periods
      this.pitchfork = pitchfork
      this.ranges = ranges
      this.ray = ray
      this.rectangle = rectangle
      this.removeButton = removeButton
      this.saveButton = saveButton
      this.searchIndicators = searchIndicators
      this.segment = segment
      this.series = series
      this.shapeOptions = shapeOptions
      this.shapes = shapes
      this.shortPeriod = shortPeriod
      this.signalPeriod = signalPeriod
      this.simpleShapes = simpleShapes
      this.slowAvgPeriod = slowAvgPeriod
      this.standardDeviation = standardDeviation
      this.stroke = stroke
      this.strokeWidth = strokeWidth
      this.style = style
      this.timeCycles = timeCycles
      this.title = title
      this.topBand = topBand
      this.tunnel = tunnel
      this.typeOptions = typeOptions
      this.verticalArrow = verticalArrow
      this.verticalCounter = verticalCounter
      this.verticalLabel = verticalLabel
      this.verticalLine = verticalLine
      this.volume = volume
      this.xAxisUnit = xAxisUnit
    })
  }
}
